...
class Percolation_exp:
    ...
    def __init__(self, n):
        ...
        self.grid = np.zeros((n, n)) 
        self.n=n
        #Initialize an array to store the parent of each site (including virtual top and bottom sites)
        self.parent = [i for i in range(n**2+2)]
        #Initialize an array to store the size of each tree (including virtual top and bottom sites)
        self.size = [1 for i in range(n**2+2)]
        #Define open sequence
        self.open_seq = random.sample(range(n**2), n**2)
        self.percolated=False
        #Connect the virtual top and bottom sites to the top and bottom rows
        for i in range(n):
            self.union(n**2, i)
            self.union(n**2+1, n**2-1-i)
    ...
    # check the Virtual top and bottom sites are connected
    def percolates(self):
        return self.connected(self.n**2, self.n**2+1) 
